...
import com.android.telephony.client.CallManager;

public class InCallUIActivity extends Activity implements View.OnClickListener,
        View.OnLongClickListener,CallManager.CallManagerListener {
    ...
    protected void onResume() {
        ...
        mCallManager.callUIActived(true);
    ...
    protected void onPause() {
        ...
        mCallManager.callUIActived(false);
...
/*            if (inCallControlState.bluetoothEnabled) {
    ...
    private void toggleHoldCall() {
        ...
        if (mCurrentScreen == SCREEN_TYPE_HOLDING) {
            mCallManager.retrieveCall();
            mDialpadButton.setEnabled(true);
        } else {
            mCallManager.holdCall();
    ...
    private void toggleMute() {
        log("toggleMute");
        mCallManager.muteMic();
    }

    private void toggleAudio() {
        log("toggleAudio");
        int state = mCallManager.getSoundRoute();
        if (state == 0)
            mCallManager.routeSound(2);
        else
            mCallManager.routeSound(0);
    }
    ...
    // Implements all the function defined by interface
    // CallManager.CallManagerListener
    public void onCallManagerServiceConnected() {
        // let Telephony Service know whether Call UI is showing or not.
        mCallManager.callUIActived(mIsResumed);
    }
